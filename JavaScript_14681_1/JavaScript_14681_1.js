...
if(x > 0){ // 만약, x가 0보다 크다면
    ...
    else{ // y가 0보다 크지 않다면(작거나 같다면(하지만, 같은 경우는 주어지지 않는다고 했다.))
        console.log("4"); // "4" 출력하기
    }
}
else{ // x가 0보다 크지 않다면(작거나 같다면(하지만, 같은 경우는 주어지지 않는다고 했다.))
    if(y > 0){ // 만약, y가 0보다 크다면
        console.log("2"); // "2" 출력하기
    }
    else{ // y가 0보다 크지 않다면(작거나 같다면(하지만, 같은 경우는 주어지지 않는다고 했다.))
        console.log("3"); // "3" 출력하기
    }
}
